...
"""
ISTORIA DIN SPATE:
Această temă va fi puțin mai diferită decât deobicei, cei de la DARWIN au apelat la serviciile noastre ca 
să-i ajutăm să organizeze produsele acestora.
Procesul lor de vindere a unui produs durează foarte mult din cauza la sistemul lor învechit de structurare 
a produselor, iar clienții sunt nemulțumiți.

Din păcate noi suntem ocupați cu procesele de organizare a Python Odyssey Camp și nu reușim la timp să 
realizăm sistemul nou. Dar în schimb voi puteți, aveți toate cunoștințele necesare.
...
"""
DARWIN de asemenea a mai menționat că unele produse au nevoie de o descriere mai detaliată a parametrilor și 
informațiilor pe care le dețin.
2. Avem nevoie de trei clase noi care să moștenească clasa `Produs`:
"""

"""
2.1Prima clasă se numește `Telefon` aceasta va moșteni clasa `Produs` și va avea doi parametri în plus numit 
`baterie_mAh` și `memorie_GB`
...
Totodată aceasta va avea o metodă numită `upgrade_battery` care va primi un parametru `new_battery` și 
va actualiza valoarea bateriei telefonului.
"""
...
"""
2.2 A doua clasă se numește `Laptop` aceasta va moșteni clasa `Produs` și va avea doi parametri în plus numit 
`sistem_de_operare` și `procesor`

De asemenea aceasta va avea o metodă numită `upgrade_processor` care va primi un parametru `new_processor` și 
va actualiza valoarea procesorului laptopului.
Totodată aceasta va avea o metodă numită `upgrade_os` care va primi un parametru `new_os` și va 
actualiza valoarea sistemului de operare al laptopului.
"""
...
"""
2.3 A treia clasă se numește `trotineta` aceasta va moșteni clasa `Produs` și va avea doi parametri în plus 
numit `viteza_maxima` și `autonomie_km`

De asemenea aceasta va avea o metodă numită `upgrade_speed` care va primi un parametru `new_speed` și 
va actualiza valoarea vitezei maxime a trotinetei.
Totodată aceasta va avea o metodă numită `upgrade_autonomy` care va primi un parametru `new_autonomy` și 
va actualiza valoarea autonomiei trotinetei.
"""
...
"""
3 DARWIN a mai lăsat o mică notiță după ei, au menționat că mulți cumpărători sunt interesați de produsele apple 
și adesea acestea le combină între ele.

Avem nevoie de o clasă nouă care să se numească `AppleProduct` care va moșteni clasa `Produs` și va avea
 un parametru în plus numit `culoare` și `produs_conectat` 
...
De asemenea va avea o metodă numită `combine_products` care va primi un parametru `product` ce va reprezenta 
un alt obiect de tip `AppleProduct` care va fi salvat în parametrul `produs_conectat`
...
iphone.combine_products(airpods) # În acest caz se va returna textul "Produsul a fost conectat cu succes" și dacă se va printa 
iphone.produs_conectat se va returna obiectul airpods
...
"""
4. DARWIN și-a mai adus aminte de o chestie, și-au dat seama că cei de la Google tot au nevoie de 
posibilitatea de a conecta produsele între ele.

Avem nevoie de o clasă nouă care să se numească `GoogleProduct` care va moșteni clasa `AppleProduct` 
posibilitățile la ambele sunt aceleași, dar va fi nevoie de o singură schimbare.
Produsul conectat trebuie să fie de tip `GoogleProduct` iar culoarea acestuia poate să fie diferită de 
a produsului curent.
Asta ar însemna că singurul element care va necesita modificări este metoda `combine_products` 
care va trebui să accepte orice tip de obiect de tip `GoogleProduct`

Exemplu utilizare:
pixel = GoogleProduct("Pixel", 10000, 2020, "negru")
home = GoogleProduct("Home", 500, 2021, "alb")
pixel.combine_products(home) # În acest caz se va returna textul "Produsul a fost conectat cu succes" și 
dacă se va printa pixel.produs_conectat se va returna obiectul home
...
"""
5. Cei de la DARWIN chiar sunt uituci, și-au adus aminte peste 5 zile că vor avea nevoie și de o 
modalitate prin care să vândă produsele.

Avem nevoie de o clasă nouă pentru aceasta, ea se va numi `Magazin` și va conține doar 2 metode, 
`vinde_produs` și `returneaza_produs`
